...
class Array
  def to_json(json : JSON::Builder, groups : Array(String), expand : Array(String))
    json.array do
      each &.to_json json, groups, expand
    end
  end
end
...
class Hash
  def to_json(json : JSON::Builder, groups : Array(String), expand : Array(String))
    json.object do
      each do |key, value|
        json.field key do
          value.to_json json, groups, expand
        end
      end
    end
  end
...
struct Tuple
  def to_json(json : JSON::Builder, groups : Array(String), expand : Array(String))
    json.array do
      {% for i in 0...T.size %}
        self[{{i}}].to_json json, groups, expand
      {% end %}
    end
  end
end
    ...
    json.object do
      {% for key in T.keys %}
        json.field {{key.stringify}} do
          self[{{key.symbolize}}].to_json json, groups, expand
        end
      {% end %}
    end
  end
