...
module YAML::Nodes
  class Document < Node
    def to_yaml(builder : YAML::Builder, groups : Array(String), expand : Array(String))
      nodes.each &.to_yaml builder, groups, expand
    end
  end
  ...
  class Mapping < Node
    def to_yaml(builder : YAML::Builder, groups : Array(String), expand : Array(String))
      builder.mapping(anchor, tag, style) do
        each do |key, value|
          key.to_yaml builder, groups, expand
          value.to_yaml builder, groups, expand
        end
      end
    end
  end
  ...
  class Sequence < Node
    def to_yaml(builder : YAML::Builder, groups : Array(String), expand : Array(String))
      builder.sequence(anchor, tag, style) do
        each &.to_yaml builder, groups, expand
      end
    end
  end
...
class Object
  ...
  #
  # Optionally, only serialize properties that are in *groups*.  See [the docs](https://github.com/Blacksmoke16/CrSerializer/blob/master/docs/serialization.md#serialization-groups) for more information.
  #
  ...
  def to_yaml(groups : Array(String) = ["default"], expand : Array(String) = [] of String) : String
    String.build do |str|
      to_yaml str, groups, expand
    end
  end

  # :nodoc:
  def to_yaml(io : IO, groups : Array(String), expand : Array(String))
    nodes_builder = YAML::Nodes::Builder.new
    to_yaml nodes_builder, groups, expand
    YAML.build(io) do |builder|
      nodes_builder.document.to_yaml builder, groups, expand
    end
  end
end
...
class Hash
  def to_yaml(yaml : YAML::Nodes::Builder, groups : Array(String), expand : Array(String))
    yaml.mapping(reference: self) do
      each do |key, value|
        key.to_yaml yaml, groups, expand
        value.to_yaml yaml, groups, expand
      end
    end
  end
end
...
class Array
  def to_yaml(yaml : YAML::Nodes::Builder, groups : Array(String), expand : Array(String))
    yaml.sequence(reference: self) do
      each &.to_yaml yaml, groups, expand
    end
  end
end
...
struct NamedTuple
  def to_yaml(yaml : YAML::Nodes::Builder, groups : Array(String), expand : Array(String))
    yaml.mapping do
      {% for key in T.keys %}
        {{key.symbolize}}.to_yaml yaml, groups, expand
        self[{{key.symbolize}}].to_yaml yaml, groups, expand
      {% end %}
    end
  end
end
